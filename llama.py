...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        # todo
        x2 = x * x
        ms_x2 = x2.mean(dim=-1, keepdim=True)
        rms = torch.sqrt(ms_x2 + self.eps)
        return x / rms
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(
        ...
        attn_weights = torch.softmax(
            attn_weights, dim=-1
        )  # (bs, n_local_heads, q_len, seq_len)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # todo
        norm_x = self.attention_norm(x)  # 1
        attn_output = self.attention(norm_x)  # 2
        attn_output = attn_output + x  # 3
        norm_attn_output = self.ffn_norm(attn_output)  # 4
        ffn_output = self.feed_forward(norm_attn_output)  # 5
